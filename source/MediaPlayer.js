...
enyo.kind({
	name: "enyo.MediaPlayer",
	kind: "enyo.Control"
});

enyo.kind({
	name: "TestControls",
	kind: "moon.MediaPlayerControls",
	components: [
...
enyo.kind({
    ...
    bindings: [
		...
		{from: ".showPlayPauseControl",		to:".$.playPause.showing"},
		{from: ".disabled",                 to:".$.jumpBack.disabled"},
		{from: ".disabled",                 to:".$.rewind.disabled"},
		{from: ".disabled",                 to:".$.playPause.disabled"},
		{from: ".disabled",                 to:".$.fastForward.disabled"},
		{from: ".disabled",                 to:".$.jumpForward.disabled"}
    ],
...
enyo.kind({
	name: "moon.MediaPlayerControls",
	kind: "enyo.MediaPlayerControls",
    ...
    observers: {
    	updatePlayPauseButtons: ["playIcon", "pauseIcon", "playing"],
    	updateMoreButton: ["moreControlsIcon", "lessControlsIcon"]
    },
	baseComponents: [		
		...
		{name: "controlsContainer", kind: "Panels", arrangerKind: "CarouselArranger", fit: true, draggable: false, classes: "moon-video-player-controls-container", components: [
			{name: "trickPlay", ontap:"playbackControlsTapped", components: [
				{name: "playbackControls", classes: "moon-video-player-control-buttons", defaultKind: "moon.IconButton", components: [
					{name: "jumpBack",		xkind: "moon.IconButton", small: false, onholdpulse: "jumpBackHeld", ontap: "doRequestJumpBack"},
					{name: "rewind",		xkind: "moon.IconButton", small: false, ontap: "doRequestRewind"},
					{name: "playPause",		xkind: "moon.IconButton", small: false, ontap: "playPauseTapped"},
					{name: "fastForward",	xkind: "moon.IconButton", small: false, ontap: "doRequestFastForward"},
					{name: "jumpForward",	xkind: "moon.IconButton", small: false, onholdpulse: "jumpForwardHeld", ontap: "doRequestJumpForward"}
				]}
			]},
	...
	initComponents: function() {
		this.createComponents(this.baseComponents, {isChrome: true});
		if (this.kindComponents && this.kindComponents.length) {
			this.addCustomControls(this.kindComponents, {isChrome: true});
		}
		else if (this.components && this.components.length) {
			this.addCustomControls(this.components);
		}
		else {
	...
	addCustomControls: function(inComponents, isChrome) {
		var props = {
			owner: isChrome ? this : this.getInstanceOwner(),
			isChrome: isChrome
		}
		inComponents = (inComponents) ? enyo.clone(inComponents) : [];
		this.customControlsCount = inComponents.length;
		if (inComponents.length <= 2) {
			// One or two components - destroy more button and utilize left/right premium placeholders
			this.$.leftPremiumPlaceHolder.createComponent(inComponents.shift(), props);
			if (inComponents.length === 1) {
				this.$.rightPremiumPlaceHolder.createComponent(inComponents.shift(), props);
			}
		} else {
		...
		this.createComponents(inComponents, props);
	},
	showPlaybackControlsChanged: function(inOld) {
		this.$.trickPlay.set("showing", this.showPlaybackControls);
		this.$.moreButton.set("showing", this.showPlaybackControls && this.customControlsCount > 2);
	...
	spotFSBottomControls: function() {
		if (this.showPlaybackControls) {
			if (this.$.controlsContainer.getIndex() === 0) {
				if (enyo.Spotlight.spot(this.$.playPause) === false) {
	...
	*/
	...
	updatePlayPauseButtons: function() {
		var t = this.bindSafely("transformIconSrc");
		this.$.playPause.setSrc(this.playing ? t(this.pauseIcon) : t(this.playIcon));
	},
	playPauseTapped: function() {
		if (this.playing) {
			this.doRequestPause();
		}
		else {
	...
	_holdPulseThreshold: 400,
	/*_holding: false,
	_sentHold: false,*/

	jumpBackHeld: function(inSender, inEvent) {
		if (inEvent.holdTime > this._holdPulseThreshold) {
	...
	jumpForwardHeld: function(inSender, inEvent) {
		if (inEvent.holdTime > this._holdPulseThreshold) {
			if (inSender._sentHold !== true) {
...
enyo.kind({
	...
	//* @protected
	handlers: {
		onRequestPlay: "playPause",
		onRequestPause: "playPause",
		onRequestFastForward: "fastForward",
		onRequestRewind: "rewind",
		onRequestJumpForward: "onjumpForward",
		onRequestJumpBack: "onjumpBackward",
    ...
    bindings: [
		...
		{from: "._isPlaying",				to:".$.controls.playing"}
    ],
	...
	updatePlaybackControlState: function() {
		...
		this.$.controls.set("disabled", disabled);
	...
	updatePlayPauseButtons: function() {
		...
		this.$.controls.set("playing", this._isPlaying);
