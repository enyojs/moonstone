...
enyo.kind({
	...
	getILibDate: function(params){
		if (this.ilibLocaleInfo.locale.spec === "th-TH") {
	...
	updateHour: function(inDate, inHour) {
		...
		var hour = this._hf ? this._hf.format(this.getILibDate({unixtime: inDate.getTime(), timezone:"UTC"})) : inHour;
		this.$.hour.setContent(hour);
	},
	updateMinute: function(inDate, inHour) {
		var time = this._mf ? this._mf.format(this.getILibDate({unixtime: inDate.getTime(), timezone:"UTC"})) : this._formatNumber(inDate.getMinutes());
		var meridiem = "";
	...
	updateMonthDay: function(inDate) {
		var md = this._mdf ? this._mdf.format(this.getILibDate({unixtime: inDate.getTime(), timezone:"UTC"})) : this.months[inDate.getMonth()] + " " + this._formatNumber(inDate.getUTCDate());
