...
enyo.kind({
	...
	animateTo : function(inStartValue, inEndValue) {
		inEndValue = this.clampValue(this.min, this.max, inEndValue);
		// Moved from animatorStep
	...
	valueChanged : function(preValue, inValue) {
		if (!this.dragging) {
			if (this.constrainToBgProgress) {
				inValue = this.clampValue(this.min, this.bgProgress, inValue);
				// Moved from animatorStep
	...
	updateKnobPosition : function(inValue) {
		var percent = this.calcPercent(inValue), knobValue = (this.showPercentage && this.popupContent === null) ? percent : inValue;
	...
	spotLeft : function(inSender, inEvent) {
		if (this.selected) {
			...
			var v = this.rtl ? this.getValue() + (this.increment || 1) : this.getValue() - (this.increment || 1);
