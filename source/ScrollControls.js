	...
	enyo.kind(
		...
		*/
		components: [
			{name: 'vColumn', classes: 'moon-scroller-v-column', components: [
				{name: 'pageUpControl', kind: 'moon.NewPagingControl', disabled: true, ontap: 'pageUp', defaultSpotlightDisappear: 'pageDownControl', defaultSpotlightDown: 'pageDownControl', side: 'top', onSpotlightUp: 'spotPaging'},
				{name: 'vthumbContainer', classes: 'moon-scroller-thumb-container moon-scroller-vthumb-container', components: [
					{name: 'vthumb', kind: 'enyo.NewScrollThumb', classes: 'moon-scroller-vthumb hidden', axis: 'v', minSize: 20}
				]},
				{name: 'pageDownControl', kind: 'moon.NewPagingControl', disabled: true, ontap: 'pageDown', defaultSpotlightDisappear: 'pageUpControl', defaultSpotlightUp: 'pageUpControl', side: 'bottom', onSpotlightDown: 'spotPaging'}
			]},
			{name: 'hColumn', classes: 'moon-scroller-h-column', components: [
				{name: 'pageLeftControl', kind: 'moon.NewPagingControl', disabled: true, ontap: 'pageLeft', defaultSpotlightDisappear: 'pageRightControl', defaultSpotlightRight: 'pageRightControl', side: 'left', onSpotlightLeft: 'spotPaging'},
				{name: 'hthumbContainer', classes: 'moon-scroller-thumb-container moon-scroller-hthumb-container', components: [
					{name: 'hthumb', kind: 'enyo.NewScrollThumb', classes: 'moon-scroller-hthumb hidden', axis: 'h', minSize: 20}
				]},
				{name: 'pageRightControl', kind: 'moon.NewPagingControl', disabled: true, ontap: 'pageRight', defaultSpotlightDisappear: 'pageLeftControl', defaultSpotlightLeft: 'pageLeftControl', side: 'right', onSpotlightRight: 'spotPaging'}
			]},
		...
		*/
		rendered: enyo.inherit(function (sup) {
			return function() {
				var measure = this.measureScrollColumns && !moon.ScrollControls._scrollColumnsMeasured;
		...
		*/
		_measureScrollColumns: function() {
			var cs;
			cs = enyo.dom.getComputedStyle(this.parent.hasNode());
			moon.ScrollControls.vScrollColumnSize =
				parseInt(cs['padding-left'], 10) +
				parseInt(cs['padding-right'], 10);
			moon.ScrollControls.hScrollColumnSize = this.$.hColumn.hasNode().offsetHeight;
			moon.ScrollControls._scrollColumnsMeasured = true;
		},
		...
		*/
		enableDisableVerticalScrollControls: function(enabled) {
			var hOffProp = this.rtl ? 'leftOffset' : 'rightOffset',
				hOffVal = enabled ? moon.ScrollControls.vScrollColumnSize : 0;
		...
		*/
		enableDisableHorizontalScrollControls: function(enabled) {
			var bOffVal = enabled ? moon.ScrollControls.hScrollColumnSize : 0;
