/**
	_sun.CalendarPicker_ is a control that displays a monthly calendar, with the
	...
	{kind: "sun.CalendarPicker", content: "Calendar Title"}
*/
enyo.kind({
	name: "sun.CalendarPickerDate",
	kind: "sun.Button",
	marquee: false,
...
enyo.kind({
	...
	setupCalendar: function() {
		if (!this.$.dates.controls.length) {
			for (var i = 1; i <= this.maxWeeks * 7; i++) {
				this.$.dates.createComponent({kind: "sun.CalendarPickerDate"});
			}
		}
	},
	/**
	...
	*/
	...
	selectDate: function(inSender, inEvent) {
		if (inEvent.originator.kind == "sun.CalendarPickerDate") {
			var newValue = inEvent.originator.value;
			this.setValue(newValue);
		} else if ( inEvent.originator.owner.kind == "sun.CalendarPickerDate") {
