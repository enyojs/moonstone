...
if (platform.touch) {
	...
	*/
	module.exports = kind(
		...
		*/
		keyup: function (inSender, inEvent) {
			...
			var strategy = this.getStrategy(),
				...
				pageUpKeyDirection,
				pageDownKeyDirection,
				pageUpKeyCtr,
				pageDownKeyCtr;

			if (showVertical) {
				pageUpKeyDirection = 'up';
				pageDownKeyDirection = 'down';
				pageUpKeyCtr = strategy.$.pageUpControl;
				pageDownKeyCtr = strategy.$.pageDownControl;
			} else {
				pageUpKeyDirection = 'left';
				pageDownKeyDirection = 'right';
			...
			if (Spotlight.getCurrent()) {
				...
				if (inEvent.keyCode === KEY_POINTER_PAGE_UP) {
					var oControl = Spotlight.NearestNeighbor.getFarthestNeighbor(pageUpKeyDirection, Spotlight.getCurrent(), viewportBounds);
					if (!oControl) {
						viewportBounds = this.getNextViewportBounds(viewportBounds, pageUpKeyDirection);
						oControl = Spotlight.NearestNeighbor.getFarthestNeighbor(pageUpKeyDirection, Spotlight.getCurrent(), viewportBounds);
					}
					if (oControl) {
					...
					var oControl = Spotlight.NearestNeighbor.getFarthestNeighbor(pageDownKeyDirection, Spotlight.getCurrent(), viewportBounds);
					if (!oControl) {
						viewportBounds = this.getNextViewportBounds(viewportBounds, pageDownKeyDirection);
						oControl = Spotlight.NearestNeighbor.getFarthestNeighbor(pageDownKeyDirection, Spotlight.getCurrent(), viewportBounds);
					}
					if (oControl) {
						Spotlight.spot(oControl);
					} else {
		...
		* @private
		*/
		getNextViewportBounds: function (viewportBounds, pageUpKeyDirection) {
			switch (pageUpKeyDirection) {
				case 'up':
