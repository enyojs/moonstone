...
*
* The item may define a
* [badgeIconPosition]{@link moon.BadgeOverlaySupport#badgeIconPosition} to
* specify position of badgeIcon. Possible values are 'left' or 'right'. Defaulted as 'left'.
*
* ```javascript
* {name: 'list', kind: 'moon.DataList', components: [
*	{mixins: ['moon.BadgeOverlaySupport'], badgeType: 'video', position: 'top', badgeIconPosition: 'right',
*		kind: 'moon.ImageItem', bindings: [
*			{from: '.model.title', to: '.label'},
*			{from: '.model.description', to: '.text'},
*			{from: '.model.coverSource', to: '.source'}
...
module.exports = {
	...
	*/
	create: kind.inherit(function (sup) {
		return function () {
			this.kindComponents = Component.overrideComponents(this.kindComponents, this._badgeOverride);
	...
	*/
	dispatchEvent: kind.inherit(function (sup) {
		return function (nom, ev, sender) {
			if (ev && !ev.delegate) {
				if (nom == 'ontap') {
					if (this.badgeTap(sender, ev)) {
						return true;
					}
				}
			}
	...
	*/
	badgeTap: function (sender, ev) {
		if (ev && ev.dispatchTarget) {
			var target = ev.dispatchTarget;
			if (target.isDescendantOf(sender.$.badgeScrimIcon)) {
				this.bubble('onBadgeIconTap', ev);
				return true;
			} else if (target.isDescendantOf(this.$.image)) {
				this.bubble('onBadgeTap', ev);
				return true;
			}
		}
		return false;
