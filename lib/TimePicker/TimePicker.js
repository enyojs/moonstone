...
var TimePicker = module.exports = kind(
		...
		for(f = 0, l = doneArr.length; f < l; f++) {
			...
			case 'k':
				pickers.push(this.wrapComponent(
					...
					{kind: Control, classes: 'moon-date-picker-wrap', components:[
						{name: 'hour', kind: HourPicker, formatter: this.hourFormatter || this, value: values.hour, onChange: 'pickerChanged'},
			...
			case 'm':
				pickers.push(this.wrapComponent(
					{name: 'timeWrapper', kind: Control, classes: 'moon-time-picker-wrap'},
					{kind: Control, classes: 'moon-date-picker-wrap', components:[
						{name: 'minute', kind: MinutePicker, formatter: this.minuteFormatter || this, value: values.minute, onChange: 'pickerChanged'},
	...
	*/
	pickerChanged: function (sender, event) {
		...
		if (this.value) {
			if (sender == this.$.hour) this.updateValue(hour);
	...
	*/
	setChildPickers: function (inOld) {
		...
		if (value) {			
			...
			if (this.meridiemEnable) this.$.meridiem.setValueByTime(value.getHours(), value.getMinutes());
		}
