...
var TimePicker = module.exports = kind(
		...
		for(f = 0, l = doneArr.length; f < l; f++) {
			...
			case 'a':
				if (this.meridiemEnable === true) {
					...
					this.$.meridiem.value = this.$.meridiem.getMeridiemIndex(valueHours, valueMinutes);
				}
				break;
			default:
				break;
	...
	*/
	hourPickerChanged: function (sender, event) {
		...
		if (this.value) this.updateValue('hour', hour);
	...
	*/
	setChildPickers: function (inOld) {
		if (this.value) {
			...
			if (this.meridiemEnable === true) this.$.meridiem.set('value', this.$.meridiem.getMeridiemIndex(values.hour, values.minute));
		}
	...
	*/
	valueChanged: function (old) {
		if (typeof ilib !== 'undefined') this.localeValue = ilib.Date.newInstance({unixtime: this.value.getTime(), timezone: 'local'});
