...
module.exports = kind(
	...
	*/
	components: [
		{name: 'buttonLeft',  kind: IconButton, backgroundOpacity: 'transparent', classes: 'moon-simple-picker-button left', icon:'arrowlargeleft', onSpotlightSelect: 'left', ondown: 'downLeft', onholdpulse:'left', defaultSpotlightDisappear: 'buttonRight', accessibilityLabel: $L('press ok button to change the value')},
		...
		{name: 'buttonRight', kind: IconButton, backgroundOpacity: 'transparent', classes: 'moon-simple-picker-button right', icon:'arrowlargeright', onSpotlightSelect: 'right', ondown: 'downRight', onholdpulse:'right', defaultSpotlightDisappear: 'buttonLeft', accessibilityLabel: $L('press ok button to change the value')}
	],
	...
	*/
	ariaObservers: [
		// Since we're toggling aria-hidden on the children, we want the reader to treat changes as
		// a unit. The result should be that the new value is read along with the label on the arrow
		// buttons.
		{to: 'aria-atomic', value: true},

		// Delegate the label to the controls used as items knowing that only the seleted control
		// will be enabled
		{path: 'generated', method: function () {
			this.setAriaAttribute('aria-labelledby', this.$.client.id);
		}},

		// ensure that only the selected option is enabled for accessibility
		{path: 'selected', method: function () {
			var options = this.getClientControls(),
				l = options.length;
			while (--l >= 0) options[l].set('accessibilityDisabled', options[l] != this.selected);
		}}
	]
});
