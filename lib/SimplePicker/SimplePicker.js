...
var MarqueeGroupItem = kind(
	/** @lends module:moonstone/SimplePicker~MarqueeGroupItem.prototype */ {

		/**
		 * @private
		 */
		name: 'moon.MarqueeGroupItem',
...
module.exports = kind(
	...
	*/
	components: [
		...
		{name: 'clientWrapper', kind: Control, classes:'moon-simple-picker-client-wrapper', components: [
			{name: 'client', kind: Group, classes: 'moon-simple-picker-client'}
		]},
	...
	*/
	 ...
	 * @private
	 */
	syncSelected: function (sender, e) {
		this.set('selected', e.active);
	},
	...
	initializeActiveItem: function() {
		...
		for (i = 0; i < controls.length; i++) {
			if (controls[i].active) {
				...
				this.selected = this.$.client.active = controls[i];
