...
module.exports = kind(
	...
	*/
	createJumpIncrementButton: function() {
		this.createComponent({name: 'buttonLeft', kind: IconButton, noBackground:true, classes: 'moon-simple-picker-button left', icon:'arrowlargeleft', onSpotlightKeyDown:'configureSpotlightHoldPulse', onSpotlightSelect: 'left', ondown: 'downLeft', onholdpulse:'left', defaultSpotlightDisappear: 'buttonRight'});
		this.createComponent({name: 'sliderWrapper', classes: 'moon-slider', spotlight: true});
		this.createComponent({name: 'buttonRight', kind: IconButton, noBackground:true, classes: 'moon-simple-picker-button right', icon:'arrowlargeright', onSpotlightKeyDown:'configureSpotlightHoldPulse', onSpotlightSelect: 'right', ondown: 'downRight', onholdpulse:'right', defaultSpotlightDisappear: 'buttonLeft'});
	...
	*/
	spotLeft: function(sender, e) {
		if (this.selected) {
				...
				if (this.enableJumpIncrement) {
					if (e.originator.isDescendantOf(this.$.buttonRight)) {
						return false;
					} else if (e.originator.isDescendantOf(this.$.buttonLeft)) {
						increment = this.jumpIncrement;
					}
				} else {
					increment = this.increment;
				}
			// If in the process of animating, work from the previously set value
			v = this.getValue() - (increment || 1);
