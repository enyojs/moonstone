...
module.exports = kind(
	...
	*/
	initILib: function () {
		...
		if (this.value) {
			this.localeValue = DateFactory.newInstance({unixtime: this.value.getTime(), timezone: "local"});
		}
	},
	 ...
	 */
	getMinYear: function() {
		var greg = DateFactory.newInstance({
			type: "gregorian",
			year: this.minYear,
			month: 1,
			day: 1,
		...
		var localCalendarDate = DateFactory.newInstance({
	...
	*/
	pickerChanged: function (inSender, inEvent) {
		...
		this.localeValue = DateFactory.newInstance({
