...
exports.Support = {
	...
	* @private
	*/
	jumpBackward: function (sender, event) {
	...
	* @private
	*/
	jumpForward: function (sender, event) {
	...
	* @public
	*/
	toggleShuffle: function (shuffleOrder, playOnShuffle) {
		this.set('shuffle', !this.shuffle);
		if (this.shuffle) {
			if (shuffleOrder) {
				this.shufflePlayOrder(playOnShuffle);
			}
	...
	* @protected
	*/
	shufflePlayOrder: function (forcePlay) {
		var c = this.get('collection'),
			length = c ? c.length : 0,
			idx = length,
			tmp, r;
		...
		if (forcePlay && c && this.playOrder[0] !== undefined) {
