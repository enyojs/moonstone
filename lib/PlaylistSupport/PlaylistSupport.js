...
exports.Support = {
	...
	*/
	next: function (sender, event) {
		...
		if (!nextModel || (this.repeat == 'none' && this.get('model') == nextModel)) {
			this.stop();
			this.onLastPlayEnd(sender, event);
			return;
		}
	...
	*/
	...
	/**
	* Called when play is ended for the last media.
	* User can override this virtual handler to handle something.
	*
	* @protected
	*/
	onLastPlayEnd: function (sender, event) {
		this.log('last');
	}
};